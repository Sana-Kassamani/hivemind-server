...
import { Comment, Task } from './schema/task.schema';
...
export class TasksService {
  ...
  async getTasks(apiaryId: string) {
    const apiary = await this.apiaryModel
      ...
      .populate({ path: 'tasks.comments.userId' });
    return apiary;
  }
  ...
  async completeTask(apiaryId: string, taskId: string) {
  ...
  async addComment(
    ...
    const newComment = new Comment(updateTaskDto);
    // access a task in an apiary
    const updatedApiary = await this.apiaryModel.findOneAndUpdate(
      { _id: apiaryId, 'tasks._id': taskId },
      {
